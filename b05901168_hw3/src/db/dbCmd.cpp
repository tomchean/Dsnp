   ...
   if (!dbjson == false) {
        ...
        if (isValidVarName(options[0])){
                for (size_t i =0 ;i < dbjson.size(); i++ ){
                        if ( dbjson[i].key() == options[0]) repeat =true;
                }
                if (repeat == false){
                        if(myStr2Int(options[1],value)){
                                DBJsonElem apend(options[0],value);
                                dbjson.add(apend);
                        }
                        else  return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[1]);
                }
                else cerr << "Error: Element with key \"" << options[0]<< "\" already exists!!"<<endl;
        }
        else return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[0]);   
   }    
   else cerr << "Error: DB is not created yet!!"<<endl;
...
{  
   ...
   if (!dbjson == false) {
        ...
        if (!CmdExec::lexOptions(option, options))
                return CMD_EXEC_ERROR;
        if (options.empty()){  //if no option print entire json
                cout<<"{\n";
                if (dbjson.size() >= 1){
                        ...
                        cout<< "  \""<< dbjson[dbjson.size()-1].key() <<"\" : " << dbjson[dbjson.size()-1].value()<<"\n";
                }
        
                cout<<"}\nTotal JSON elements: "<<dbjson.size()<<endl;
        }
                ...
                string extra;
                size_t n = myStrGetTok(option, extra);
                extra = option.substr(n);               
                return CmdExec::errorOption(CMD_OPT_EXTRA,extra);
        }
        }
   else cerr << "Error: DB is not created yet!!"<<endl;
   return CMD_EXEC_DONE;

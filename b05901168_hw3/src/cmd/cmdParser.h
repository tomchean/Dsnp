...
public:
   ...
   int StrCmp(const string& s1, const string& s2, unsigned n); //self-define
   bool mylexOptions(const string& option, vector<string>& tokens, size_t nOpts) const ; //self-define
   bool cmpfiles(const vector<string>& files, int k); //self-define
   void printfile(const vector<string>& files); //self-define
   int  match(vector<map<const string, CmdExec*>::const_iterator> & temp , const string cmd); //self-define

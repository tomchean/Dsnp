...
#define toSizeT(t)     t+((-t)%SIZE_T)   // TODO
//
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      for (size_t t=0;t<R_SIZE;t++){
        _recycleList[t].reset();
      }
      if (b ==0 || b == _blockSize) _activeBlock->reset();
      else {
        _blockSize = b ;
        delete _activeBlock;
        _activeBlock = new MemBlock<T> (0,_blockSize);
...
private:
   ...
   size_t getArraySize(size_t t) const {
      ...
      assert(t >= S);
      // TODO
      if (t < toSizeT(S+SIZE_T)) return 0;
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      if (ret->getArrSize() == n) return ret;
      while (ret->_nextList !=0){
        ret = ret->_nextList;
        if (ret->getArrSize() == n ) return ret;
      }
      MemRecycleList<T>* last = new MemRecycleList<T>(n);
      ret->setNextList(last);
      return last;   
   }
   ...
   T* getMem(size_t t) {
      ...
      if (ret!=0){
        ...
        //no match
        if (! _activeBlock->getMem(t,ret) ){ //get from activeblock
            //fail to get from activeblock
          if(_activeBlock->getRemainSize() >= toSizeT(S)){
            size_t index =getArraySize(_activeBlock->getRemainSize());
            if (index ==0)  _activeBlock ->getMem(toSizeT(S),ret);
            else _activeBlock ->getMem(toSizeT(index*S+SIZE_T),ret);
            #ifdef MEM_DEBUG
            cout << "Recycling " << ret << " to _recycleList[" << index << "]\n";
            #endif // MEM_DEBUG
            getMemRecycleList(index)->pushFront(ret);
          }
